...
export class Ng2SliderComponent implements ISkinable {
    ...
    initialStartValue: any = null;
    initialEndValue: any = null;
    ...
    initNormalHandlerStyle = {}
    initSlidingHandlerStyle = {};
    /* initRangeRibbonStyle = {
        left: '0%',
        width: '100%',
        height: '10px',
        border: 'solid 1px',
    ...
    resultNormalHandlerStyle = {};
    resultSlidingHandlerStyle = {};
    resultRangeRibbonStyle: any = {};
    resultHandleStyle = [];
    ...
    refreshInputBoxByPercent(percent, handle: RangeHandle, event) {
        let precision = this.calculatePrecision(this.stepValue)
        let value = (+this.min + (this.max - this.min) * percent / 100).toFixed(precision);
        switch (handle) {
            case RangeHandle.Start:
                this.startValue = value.toString();
                $('#left-side').text(this.startValue).css({ left: event.boundingRect.left + 'px', top: event.boundingRect.top - 30 + 'px' });
                $('#date-range').css({ left: event.boundingRect.left + 20 + 'px', top: event.boundingRect.top + 5 + 'px' });
                break;
            case RangeHandle.End:
                this.endValue = value.toString();
                $('#right-side').text(this.endValue).css({ left: event.boundingRect.left + 'px', top: event.boundingRect.top - 30 + 'px' });
                $('#date-range').css({ right: ($(window).width() - event.boundingRect.left) + 'px', top: event.boundingRect.top + 5 + 'px' });
                break;
            default:
                break;
        }
    ...
    onSliding(event: IEventSlideAble) {
        ...
        this.refreshInputBoxByPercent(event.relativePercentHorisontal, handle, event);
    }
