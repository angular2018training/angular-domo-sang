...
onDownloadClick(file) {
  return this._WorkForceDataService.downloadIpMapping(file.id).then(result => {
    let a = file.fileName;
...
createAction(request) {
  return this._WorkForceDataService.createIpMapping(this.clientId, request).then(result => {
    if (result) {
...
getWorkForceDataList() {
  return this._WorkForceDataService.getAllMappingData(this.clientId).then(result => {
    if (result) {
      this.fileId = result.id;
      this.data = result;
...
deleteWorkForceData(fileId) {
  return this._WorkForceDataService.deleteIpMapping(fileId).then(result => {
    if (result) {
