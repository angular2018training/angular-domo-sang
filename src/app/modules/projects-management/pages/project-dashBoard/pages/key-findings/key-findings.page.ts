...
@Component({
  ...
  templateUrl: './key-findings.page.html',
  styleUrls: ['./key-findings.page.scss'],
  host: {
    '(document:keydown)': 'handleKeyDownEvents($event)',
    '(document:keyup)': 'handleKeyUpEvents($event)'
  }
})
export class KeyFindingsPage implements OnInit {
  projectId;
  ...
  maxSlide: number = 64;
  // isSleclectd = this.slides.selected : false;
  ...
  subTitleContent = '{Subtitle for Key Finding}';
